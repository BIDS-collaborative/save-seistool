/*	$Id: sachead.h,v 1.1 2001/12/21 18:39:04 lombard Exp $	*/
...
/*  2000/09/06	Doug Neuhauser, doug@seismo.berkeley.edu
    Per conversation with SAC developer Peter Goldstein,
	KHOLE header is used to store SEED location code.
	KNETWK header is used to store SEED network code.
*/
...
*	Values of `-12345' are used for any variable which is UNKNOWN.
...
long blank7[SACTEMP7];
	...
	char kstnm[K_LEN];	/* SEED station name (blank padded) */
	char kevnm[KEVNMLEN];	/* event name (UofW pickfile name) */
	char khole[K_LEN];	/* SEED location code (blank padded). */
	...
	char kuser1[K_LEN];
	char kuser2[K_LEN];
	char kcmpnm[K_LEN];	/* SEED channel name (blank padded). */
	char knetwk[K_LEN];	/* SEED network code (blank padded). */
	char kdatrd[K_LEN];
	char kinst[K_LEN];
};
